...
# --- 1. DATABASE & LOGIC ---
def init_db():
    conn = sqlite3.connect('crypto_master_v24.db', check_same_thread=False)
    c = conn.cursor()
...
def draw_market_header():
    # Fear & Greed + Market Sentiment (From Photos)
    col1, col2 = st.columns(2)
    with col1:
        st.markdown('''<div class="glass-card" style="text-align:center;">
            <small style="color:#848e9c;">Fear & Greed Index</small>
            <h2 style="color:#f0b90b; margin:0;">64 <span style="font-size:14px; color:#848e9c;">Greed</span></h2>
...
def user_portal():
    ...
    st.markdown("<h1 style='text-align:center; font-weight:900;'>CRYPTO ELITE PRO</h1>", unsafe_allow_html=True)
    draw_ticker()
    draw_market_header()
    ...
    st.markdown("""
    <div class="glass-card">
        <div style="display:flex; justify-content:space-between; margin-bottom:10px;"><span><b>BTC</b> Bitcoin</span> <span class="neon-green">$96,432.10</span></div>
        <div style="display:flex; justify-content:space-between;"><span><b>ETH</b> Ethereum</span> <span class="neon-green">$2,845.50</span></div>
    </div>
    """, unsafe_allow_html=True)
